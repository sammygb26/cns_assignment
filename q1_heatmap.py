...
def simulate(W0, W1, ax):
    ...
    ax[0].tick_params(axis='x', left = False, right = False , labelleft = False , 
                labelbottom = False)
    fig.colorbar(im1, ax=ax[0], aspect=14, label="$V_i$", pad=0.01)
    ...
    ax[1].tick_params(axis='x', left = False, right = False , labelleft = False , 
                labelbottom = False)
    fig.colorbar(im2, ax=ax[1], aspect=14, label="$r_i$", pad=0.01)
    ...
    ax[2].tick_params(axis='x', left = False, right = False , labelleft = False , 
                labelbottom = False)
    fig.colorbar(im3, ax=ax[2], aspect=14, label="$n_i$", pad=0.01)
    ...
    fig.colorbar(im3, ax=ax[3], aspect=14, label="$u_i(t)+\sqrt{\\frac{\\tau}{\\delta t}}\\sigma q_i(t)$", pad=0.01)
